...
function sendNotifications_(now, orderId, customer, items, totals, settings) {
  ...
  const subject = "NEW WHOLESALE ORDER — " + orderId + " — " + safeSubject_(customer.company);
  const body = buildSalesEmailText_(now, orderId, customer, items, totals);
...
function buildSalesEmailText_(now, orderId, customer, items, totals) {
  return [
    "NEW WHOLESALE ORDER", "", "Order: " + orderId, "Received: " + now,
    ...
    "Tax province: " + totals.provinceName + (totals.province ? " (" + totals.province + ")" : ""),
    customer.poNumber ? "PO: " + customer.poNumber : "",
    customer.notes ? "Notes: " + customer.notes : "", "", buildSummary_(items), "",
...
function buildCustomerEmailText_(orderId, customer, items, totals, settings) {
  return [
    "DESIGNATED DRINKS", "Wholesale order received", "", "Hi " + customer.contact + ",", "",
    "We recorded the wholesale order for " + customer.company + ".", "Order: " + orderId, "",
    buildSummary_(items), "", "Total cases: " + totals.totalCases,
    "Subtotal: " + formatMoney_(totals.subtotal),
    taxSummaryLabel_(totals) + ": " + formatMoney_(totals.tax),
    "Estimated total: " + formatMoney_(totals.total), "", settings.shipping_message,
