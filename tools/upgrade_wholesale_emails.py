...
old = '  PRODUCT_CACHE_SECONDS: 30,\n  VERSION: "8.0"\n'
new = '  PRODUCT_CACHE_SECONDS: 30,\n  BRAND_LOGO_URL: "https://designateddrinks.github.io/designated-wholesale/dd-logo.png",\n  BRAND_WEBSITE: "https://designateddrinks.ca",\n  VERSION: "8.1"\n'
if s.count(old) != 1:
    raise SystemExit(f'CONFIG anchor count: {s.count(old)}')
s = s.replace(old, new, 1)

old_item = '''      brand: product.brand,
      name: product.name,
...
if s.count(old_item) != 1:
    raise SystemExit(f'priced-item anchor count: {s.count(old_item)}')
s = s.replace(old_item, new_item, 1)

start = s.index('function sendNotifications_(now, orderId, customer, items, totals, settings) {')
end = s.index('\nfunction logEvent_(', start)
replacement = r'''function sendNotifications_(now, orderId, customer, items, totals, settings) {
  const warnings = [];
